// ¾ view derived FROM the front pose (TypeScript port for the app).
//
// Mirrors scripts/turn34.mjs — keep the two in sync. The same cute front cat
// with small, controlled asymmetries (features shifted toward the facing side,
// far eye narrowed, ears offset, tail visible on the near side), so every turn
// keyframe stays on-model. t = 0 -> exactly the front idle; t = 1 -> strongest
// ¾ (face angled toward viewer-right; flip the frame for left).
//
...
export function generate34Grid(preset: Pet, t: number, state: State34 = {}): Parts {
  ...
  // FORESHORTENED — a short forearm from the shoulder ending in a BIG pad that
  // faces the viewer, toe beans out. Reaching = the pad growing toward you.
  const pawAmt = state.paw || 0, pawX = state.pawX || 0
  const pawMask = new Uint8Array(W * H)
  const pawReach = 0.55 + 0.45 * pawAmt // pad size: it's the closest thing to camera
