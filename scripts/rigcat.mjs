...
export function generateRigGrid(pet, pose) {
  ...
  const faceSign = hcx >= bcx ? 1 : -1
  if (g.snout > 0) ellipse(set, hcx + faceSign * hr * 0.82, hcy + hr * 0.3, g.snout, g.snout * 0.62) // dog muzzle
  ...
  if (face >= 0.75) {
    ...
    if (pose.eye > 0.5) {
      ...
      if (es === 'sleepy') for (const dx of [-1, 0, 1, 2]) put(overlay, Math.round(hcx + 1 * kh + dx), Math.round(hcy - 0.5 - iry), O.OUTLINE)
    } else {
