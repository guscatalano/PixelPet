...
function floppyEarPts(g, s) {
  const earBaseY = g.headCy - g.headRy * 0.55
  const half = g.earW / 2
  const bx = g.headCx + s * g.earSpread
  return {
    topI: [bx - s * half * 0.2, earBaseY - 0.5],
    topO: [g.headCx + s * g.headRx * 0.9, earBaseY - 1.5],
    botO: [g.headCx + s * g.headRx * 1.18, g.headCy + g.headRy * 0.62],
    botI: [g.headCx + s * g.headRx * 0.66, g.headCy + g.headRy * 0.98]
  }
}
...
function drawFace(overlay, fur, g, state) {
  ...
  for (const s of [-1, 1]) {
    ...
    // Eye shape by style: round (open), almond (sleeker + slit pupil), sleepy (half-lidded).
    ...
    if (state.dilation !== undefined) { const d = state.dilation; pupRx = Math.max(0.6, g.eyeRx * (0.26 + 0.42 * d)); pRy = ery * (0.96 - 0.24 * d) }
...
export function generateWalkGrid(preset, step = 0, motion = 1, excite = 0) {
  ...
  // Lock the coat pattern to the actual body/head so it moves with the fur (essential
  // for hop/trot — anchoring to a rest position slides it off the airborne body).
